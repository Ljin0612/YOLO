"""Train a YOLOv10-s baseline on SeaShips24790 with Ultralytics."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train YOLOv10-s on SeaShips24790.")
    parser.add_argument("--data", default="configs/seaships24790.local.yaml", help="Dataset YAML path.")
    parser.add_argument("--model", default="yolov10s.pt", help="YOLOv10-s checkpoint path or model name.")
    ...
    parser.add_argument("--name", default="yolov10s_baseline", help="Run name.")
...
def warn_if_missing_local_model(model_path: str) -> None:
    ...
    if path.suffix == ".pt" and not path.exists():
        print(
            ...
            "access the internet, manually download yolov10s.pt and pass its local path "
            "with --model."
        )
